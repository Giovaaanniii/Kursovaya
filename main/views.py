...
from main.tasks import send_mail_hello

def home(request):
    if (request.user.is_authenticated):
        send_mail_hello.delay(request.user.email, 'Привет. Спасибо, что зашли на наш сайт!')
...
class ExcursionsFilter(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        return queryset.filter(
            short_task="c автобусом"
        )  # Пример фильтрации по полю 'short_task'
...
def events(request):
    ...
    keyword = request.GET.get(
        "keyword", "Новогодняя программа"
    )  # Предположим, что ключевое слово передается в GET параметре 'keyword'
    # Создаем Q-запрос для поиска записей по ключевому слову
    q_query = Q(
        task__icontains=keyword
    )  # Найти записи, у которых поле description содержит ключевое слово (без учета регистра)
    # Фильтруем события используя Q-запрос по ключевому слову
    tasks_events = Events.objects.filter(q_query)
    return render(
        request, "main/events.html", {"title": "Мероприятия", "tasks": tasks_events}
    )
...
def contacts(request):
    ...
    return render(
        request, "main/contacts.html", {"title": "Контакты", "tasks": tasks_contacts}
    )
...
def excursions(request):
    ...
    return render(
        request,
        "main/excursions.html",
        {"title": "Экскурсии", "tasks": tasks_excursions},
    )
...
def master(request):
    ...
    return render(
        request,
        "main/master-classes.html",
        {"title": "Мастер-классы", "tasks": tasks_master},
    )
...
def graduation(request):
    ...
    return render(
        request,
        "main/graduation.html",
        {"title": "Выпускные", "tasks": tasks_graduation},
    )
